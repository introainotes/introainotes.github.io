...
**[Theorem]** **Relevance Lemma for first order logic**
	...
	If for all $x \in Vars$, $\alpha_1[x] = \alpha_2[x]$, then $\mathcal{A}, \alpha_1 \models \varphi$ iff $\mathcal{A}, \alpha_2 \models \varphi$.

**Proof.** By induction on $Term$ then on $Form$.
...
At this point, you might wonder, how are these elements connected to our AI course? Within the scope of artificial intelligence, it's essential to encode knowledge in a specific format, where you can get answers fo your queries efficiently. First-order logic, which then comprises our knowledge base (KB). Databases can be regarded as instances of first-order logic representations. An integral inquiry at this juncture is whether the KB logically entails a given formula $\varphi$.

Moreover, when considering the relationship $\varphi \models \psi$, where both $\varphi$ and $\psi$ represent formulas, the underlying question is whether the models that satisfy $\varphi$ also satisfy $\psi$, that is, whether $\mathsf{models}(\varphi) \supseteq \mathsf{models}(\psi)$?
