...
$$\begin{align*} \hat{f}(u) = \hat{g}(u) + h(u) \end{align*}$$
...
$~~~~~$ The property $h(S_i) \leq Optimal(S_i)$ is known as **admissibility**. Therefore, consistency _implies_ admissibility under the assumption that $h(S_u) = 0$ where $S_u$ is goal.
